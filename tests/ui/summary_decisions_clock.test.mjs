...
test("decision queue reveals due choices, tracks selection, and submits the selected pair", async () => {
  ...
  const generatedBy = {
    ...
    sourceTitle: "Earlier <decision>",
    sourceChoiceId: "choice-<source>",
    sourceChoiceLabel: "Pause <work>",
    affectedJob: {
      jobId: "JOB-01",
      jobLabel: "Job 1",
  ...
  uiState.state.decisions[0].choices[0].developer = {
    ...
    followUp: {
      ...
      earliestDate: "July <3>",
      possibleDays: [3, 4],
      canceledOnSomeContinuations: true,
      variants: [{
        signature: "variant-<one>",
        title: "Result <one>",
        jobId: "JOB-01",
        jobLabel: "Job 1",
        jobName: "Pressure <Hull>",
  ...
  uiState.state.decisions[0].choices[1].developer = {
    ...
    followUp: {
      ...
      targets: [{
        scheduled: false,
        definitionId: "runtime-<check>",
        title: "Runtime <check>",
        jobName: "Electrical <work>",
        delayDays: 3,
        availableDay: 4,
        effectNote: "Effect determined when follow-up appears",
        possibilities: [{
          kind: "alternate",
  ...
  assert.equal(
    (body.innerHTML.match(/decision-diagnostic-follow-up/g) || []).length,
    1,
  );
  assert.doesNotMatch(body.innerHTML, /Generated by|Runtime &lt;check&gt;/);
  assert.match(body.innerHTML, /Forward follow-up: scheduled/);
  assert.match(body.innerHTML, /Later &lt;check&gt; · later-&lt;check&gt;/);
  assert.match(body.innerHTML, /Earliest: July &lt;3&gt; \(Day 3\)/);
  assert.match(body.innerHTML, /Some continuations cancel this follow-up/);
  assert.match(body.innerHTML, /Result &lt;one&gt;/);
  assert.match(body.innerHTML, /Inspect &lt;now&gt; · raw \+0\.50/);

  uiState.state.decisions[0].developer.generatedBy = generatedBy;
  uiState.devShowDiagnostics = false;
  renderDecisionQueue();
  uiState.devShowDiagnostics = true;
  renderDecisionQueue();
  assert.equal(
  ...
  assert.equal(
    (body.innerHTML.match(/Generated by/g) || []).length,
    2,
  );
  assert.equal(
    (body.innerHTML.match(/Forward follow-up: scheduled/g) || []).length,
    1,
  );
  assert.match(
    body.innerHTML,
    /decision-diagnostic-follow-up[\s\S]*Generated by[\s\S]*Forward follow-up: scheduled/,
  );
  assert.match(body.innerHTML, /Earlier &lt;decision&gt;/);
  assert.match(body.innerHTML, /Pause &lt;work&gt;/);
  assert.match(body.innerHTML, /Affected job: Pressure &lt;Hull&gt; · Job 1 · JOB-01/);
  assert.doesNotMatch(body.innerHTML, /Runtime &lt;check&gt;/);

  uiState.state.decisions[0].choices[1].developer.followUp.scheduled = true;
  uiState.state.decisions[0].choices[1].developer.followUp.targets[0].scheduled = true;
  uiState.devShowDiagnostics = false;
  renderDecisionQueue();
  uiState.devShowDiagnostics = true;
  renderDecisionQueue();
  ...
  assert.match(body.innerHTML, /Runtime &lt;check&gt;/);
  assert.match(body.innerHTML, /Alternate &lt;result&gt; \(alternate result\)/);
  assert.match(body.innerHTML, /Repair &lt;later&gt; · raw -0\.75/);
  assert.match(body.innerHTML, /Effect determined when follow-up appears/);
  assert.doesNotMatch(body.innerHTML, /Pressure <Hull>|July <3>|this <node>|Later <check>|Runtime <check>/);
