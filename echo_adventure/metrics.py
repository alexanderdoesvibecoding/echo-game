...
def update_state_metrics(state: SimulationState) -> None:
    """Accumulate unfinished work and capture the latest metric snapshot."""
    # Derive these flags from jobs instead of trusting callers to keep redundant
    # aggregate fields synchronized after every possible mutation.
...
def calculate_snapshot(state: SimulationState) -> MetricSnapshot:
    """Calculate a behavior-neutral metric view of the current state."""
    ...
    # Work for the current day is still available, hence a one-day job projects
    # to current_day rather than current_day + 1.
