...
function renderChoiceDiagnostics(card, choice) {
  ...
  return `
    <div class="decision-choice-diagnostics ${diagnostics.isPreferred ? "is-preferred" : "is-not-preferred"}">
      ...
      ${renderForwardFollowUpDiagnostics(diagnostics.followUp)}
    </div>
  `;
}

function renderGeneratedBy(card) {
  const source = card.developer?.generatedBy;
  if (!uiState.devShowDiagnostics || !source) return "";
  const job = source.affectedJob || {};
  return `
    <section class="decision-generated-by">
      <strong>Generated by</strong>
      <span>Day ${escapeHtml(source.sourceDay)} · ${escapeHtml(source.sourceTitle || source.sourceDefinitionId)}</span>
      <span>${escapeHtml(source.sourceChoiceLabel || source.sourceChoiceId)}</span>
      <span>Affected job: ${escapeHtml(job.jobName)} · ${escapeHtml(job.jobLabel)} · ${escapeHtml(job.jobId)}</span>
    </section>
  `;
}

function renderForwardFollowUpDiagnostics(followUp) {
  if (!followUp) return "";
  if (followUp.mode === "player-only") {
    return `
      <section class="decision-diagnostic-follow-up">
        <strong>Forward follow-up:</strong>
        <span>${escapeHtml(followUp.note || "Final-assembly choices do not schedule follow-ups.")}</span>
      </section>
    `;
  }
  if (!followUp.scheduled && !followUp.targets?.length) {
    return `
      <section class="decision-diagnostic-follow-up">
        <strong>Forward follow-up:</strong>
  ...
  if (followUp.mode === "runtime") {
    return renderRuntimeFollowUpDiagnostics(followUp);
  }

  const target = followUp.target || {};
  const possibleDays = (followUp.possibleDays || []).map(day => `Day ${escapeHtml(day)}`).join(", ");
  const variants = (followUp.variants || []).map(variant => `
    <li>
      <strong>${escapeHtml(variant.title)}</strong>
      <span>${escapeHtml(variant.jobName)} · ${escapeHtml(variant.jobLabel)} · ${escapeHtml(variant.jobId)}</span>
      <span>Possible on ${(variant.possibleDays || []).map(day => `Day ${escapeHtml(day)}`).join(", ") || "no reachable day"}</span>
      <ul>
        ${(variant.choices || []).map(choice => `
          <li>
            <span>${escapeHtml(choice.label)} · raw ${formatSigned(choice.rawScoreDelta)}</span>
            ${renderFutureJobChanges(choice.jobDayChanges)}
          </li>
        `).join("")}
      </ul>
    </li>
  ...
  return `
    <section class="decision-diagnostic-follow-up">
      <strong>Forward follow-up: scheduled</strong>
      <span>${escapeHtml(target.title)} · ${escapeHtml(target.definitionId)}</span>
      <span>Target: ${escapeHtml(target.jobName)} · ${escapeHtml(target.jobLabel)} · ${escapeHtml(target.jobId)} · delay ${escapeHtml(target.delayDays)} day${Number(target.delayDays) === 1 ? "" : "s"}</span>
      <span>Earliest: ${escapeHtml(followUp.earliestDate || "none")}${followUp.earliestDay ? ` (Day ${escapeHtml(followUp.earliestDay)})` : ""}</span>
      <span>Possible days: ${possibleDays || "none"}</span>
      <span>${followUp.canceledOnSomeContinuations ? "Some continuations cancel this follow-up because the originating job completes." : "No reachable continuation cancels this follow-up before it appears."}</span>
      <ul class="decision-follow-up-variant-list">${variants || "<li>No realized variant remains reachable.</li>"}</ul>
    </section>
  `;
}

function renderRuntimeFollowUpDiagnostics(followUp) {
  const targets = (followUp.targets || []).map(target => `
    <li>
      <strong>${target.scheduled ? "Will schedule" : "Will not schedule"}: ${escapeHtml(target.title)}</strong>
      <span>${escapeHtml(target.definitionId)} · ${escapeHtml(target.jobName)} · delay ${escapeHtml(target.delayDays)} day${Number(target.delayDays) === 1 ? "" : "s"} · available Day ${escapeHtml(target.availableDay)}</span>
      <span>${escapeHtml(target.effectNote)}</span>
      <ul>
        ${(target.possibilities || []).map(possibility => `
          <li>
            <strong>${escapeHtml(possibility.title)}${possibility.kind === "alternate" ? " (alternate result)" : ""}</strong>
            ${(possibility.choices || []).map(choice => `
              <span>${escapeHtml(choice.label)} · raw ${formatSigned(choice.rawScoreDelta)} · ${escapeHtml(choice.effectNote)}</span>
            `).join("")}
          </li>
        `).join("")}
      </ul>
  ...
  return `
    <section class="decision-diagnostic-follow-up">
      <strong>Forward follow-up: ${followUp.scheduled ? "scheduled" : "not scheduled"}</strong>
      <ul class="decision-follow-up-variant-list">${targets}</ul>
    </section>
  `;
}
...
function renderFutureJobChanges(changes) {
  ...
  return (changes || []).map(change => `
    <span>
      ${escapeHtml(change.jobName)} · ${escapeHtml(change.jobLabel)} · ${escapeHtml(change.jobId)}
      · ${formatSigned(change.days)} day${Math.abs(change.days) === 1 ? "" : "s"}
      · ${escapeHtml(change.remainingBefore)} → ${escapeHtml(change.remainingAfter)} remaining
    </span>
  `).join("");
}
