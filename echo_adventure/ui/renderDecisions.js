...
function renderChoiceDiagnostics(card, choice) {
  ...
  return `
    <div class="decision-choice-diagnostics ${diagnostics.isPreferred ? "is-preferred" : "is-not-preferred"}">
      ...
      ${renderChoiceFollowUpDiagnostics(card, diagnostics.followUp)}
    </div>
  `;
}
...
function renderChoiceFollowUpDiagnostics(card, followUp) {
  ...
  if (!source && !hasForwardFollowUp) return "";
  return `
    <section class="decision-diagnostic-follow-up">
      ${source ? renderGeneratedByDetails(source) : ""}
      ${hasForwardFollowUp ? renderForwardFollowUpDetails(followUp) : ""}
    </section>
  `;
}
...
function renderGeneratedByDetails(source) {
  ...
  return `
    <div class="decision-follow-up-direction is-reverse">
...
function renderForwardFollowUpDetails(followUp) {
  ...
  `).join("");
  return `
    <div class="decision-follow-up-direction is-forward">
...
function renderRuntimeFollowUpDetails(followUp) {
  const targets = (followUp.targets || []).filter(target => target.scheduled).map(target => `
    <li>
      <strong>Will schedule: ${escapeHtml(target.title)}</strong>
