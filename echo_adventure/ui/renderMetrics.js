...
export function renderMetrics() {
  ...
  const metrics = [
    ...
    ["Subjobs Complete", `${fmtNum(snap.jobsCompleted)}/${fmtNum(totalSubjobs)}`, snap.jobsCompleted / Math.max(1, totalSubjobs), "good", "Total subjobs finished out of all required work.", true, "", jobsCompletedTodayDelta],
  ...
  $("metrics").innerHTML = metrics.map(([label, value, pct, tone, tooltip, showBar, detail, liveDelta]) => `
    <div class="metric ${detail ? "hoverable" : ""} ${liveDelta ? "has-live-delta" : ""}" ${detail ? `tabindex="0" aria-describedby="jobsMetricPopover"` : ""}>
      ${liveDelta || ""}
