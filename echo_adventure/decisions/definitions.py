"""Stable named manufacturing decision definitions.

Player-facing text deliberately describes operational consequences without
exposing the internal shift arithmetic.  Exact shift ranges live in typed
effects and are resolved deterministically by the effect engine.
"""
...
def C(
    ...
    *effects: DecisionEffect,
    ...
    reschedules = sum(int(effect.params.get("count", 1)) for effect in operations if effect.kind == "reschedule")
    return DecisionChoice(
        id=label.lower().replace("'", "").replace(" ", "-").replace("/", "-")[:64],
        label=label,
        description=description,
        immediate_effects={"type": "manufacturing", "operations": [effect.kind for effect in operations]},
...
BASE_DEFINITIONS = (
    D(
        "weather", "Severe Weather", "Severe weather has closed exposed work areas, pausing affected jobs.", "workcenter",
    ...
    D(
        "workstation-breakdown", "Workstation Breakdown", "An unexpected workstation fault has stopped the job currently running there.", "workcenter",
    ...
    D(
        "materials-not-here", "Materials Have Not Arrived", "Required material has not arrived at the planned workstation.", "material",
    ...
    D(
        "echo-recommendation", "ECHO Recommendation", "ECHO recommends a disruptive, board-wide reshuffle whose benefit may not be immediately visible.", "critical",
    ...
    D(
        "worker-off-day", "Worker Took the Day Off", "The operator assigned to this job is unavailable today.", "worker",
    ...
    D(
        "calibration-drift", "Calibration drift", "Recent measurement work is suspect until the measurement chain is trusted again.", "inspection",
    ...
    D(
        "traveler-mismatch", "Traveler mismatch", "The paper traveler and digital route disagree about the next operation.", "document",
    ...
    D(
        "shared-fixture-claim", "Shared fixture claim", "Two ready jobs need the same qualified fixture at the same time.", "fixture",
    ...
    D(
        "changeover-drag", "Changeover drag", "A family changeover is taking longer than the dispatch plan allowed.", "family",
    ...
    D(
        "batch-window-opens", "Batch window opens", "A shared process window is available earlier than expected.", "batch",
    ...
    D(
        "consumables-short", "Consumables short", "A shop's working stock is too low to sustain every planned start.", "material",
    ...
    D(
        "label-printer-outage", "Label printer outage", "Completed work cannot receive controlled labels at the release desk.", "document",
    ...
    D(
        "shop-air-pressure-dip", "Shop air pressure dip", "Pneumatic tools and clamps are running below normal reliability.", "shop",
    ...
    D(
        "coolant-change-due", "Coolant change due", "A cutting or machining station is approaching its finish-quality limit.", "workcenter",
    ...
    D(
        "fod-sweep", "FOD sweep", "Foreign object debris requires a controlled-area sweep before work resumes.", "controlled",
    ...
    D(
        "handoff-window-missed", "Handoff window missed", "A cross-shop move missed the receiving station's planned intake window.", "handoff",
    ...
    D(
        "crane-reservation-conflict", "Crane reservation conflict", "A shared heavy-lift resource is committed to another shop.", "crane",
    ...
    D(
        "nesting-opportunity", "Nesting opportunity", "Compatible work can share a setup, fixture, program, or batch.", "family",
    ...
    D(
        "old-setup-sheet", "Old setup sheet", "A proven setup sheet exists for similar work, but its applicability is not yet controlled.", "document",
    ...
    D(
        "wip-crowding", "WIP crowding", "Staging space, carts, and access lanes are constraining work in progress.", "staging",
    ...
    D(
        "cleanliness-breach", "Cleanliness breach", "A controlled area failed its cleanliness check while work was open.", "controlled",
    ...
    D(
        "software-seat-conflict", "Software seat conflict", "Programming-dependent work cannot release while all software seats are occupied.", "software",
    ...
    D(
        "network-folder-offline", "Network folder offline", "Current programs and acceptance files are temporarily unreachable.", "document",
    ...
    D(
        "gauge-dispute", "Gauge dispute", "Two gauges disagree and the affected feature cannot be accepted yet.", "gauge",
    ...
    D(
        "count-variance", "Count variance", "System inventory and the floor count disagree about available stock.", "material",
    ...
    D(
        "burr-cleanup", "Burr cleanup", "Cut or machined parts need more cleanup than the downstream plan allowed.", "family",
    ...
    D(
        "cure-clock", "Cure clock", "The next dependency cannot safely begin until a process dwell completes.", "family",
    ...
    D(
        "vacuum-leak-chase", "Vacuum leak chase", "A bag, seal, or holding fixture will not maintain process pressure.", "fixture",
    ...
    D(
        "tool-crib-hold", "Tool crib hold", "Calibrated hand tools are waiting on controlled crib release.", "tool",
    ...
    D(
        "fixture-soak", "Fixture soak", "A large fixture has not reached its required process condition.", "fixture",
    ...
    D(
        "shift-overlap-bonus", "Shift overlap bonus", "An unexpected overlap creates a short window of extra coordination.", "shop",
    ...
    D(
        "waste-container-full", "Waste container full", "Affected stations cannot continue producing until waste flow is restored.", "waste",
    ...
    D(
        "preapproved-package", "Preapproved package", "One work family already has an accepted closeout package.", "document",
    ...
    D(
        "expired-stickers", "Expired stickers", "Calibration stickers on several hand tools are out of date.", "tool",
    ...
    D(
        "vendor-rep-on-site", "Vendor rep on site", "A vendor specialist is unexpectedly available for a limited window.", "workcenter",
    ...
    D(
        "training-run", "Training run", "A newer operator can qualify on live work, slowing the current station but adding flexibility.", "worker",
    ...
    D(
        "off-peak-utility-slot", "Off-peak utility slot", "An extra utility-dependent process window can be used if the board is rearranged.", "batch",
    ...
    D(
        "floor-walk-insight", "Floor walk insight", "A small method improvement can remove wasted motion from matching work.", "family",
    ...
    D(
        "wash-tank-chemistry", "Wash tank chemistry", "A preparation tank is moving out of its controlled process range.", "batch",
    ...
    D(
        "rack-shortage", "Rack shortage", "Finished and in-process parts cannot safely move or stage because racks are full.", "staging",
    ...
    D(
        "safety-drill", "Safety drill", "A required emergency drill interrupts production across the site.", "global",
    ...
    D(
        "access-badge-failure", "Access badge failure", "A secure-area badge reader is preventing normal shift-start access.", "controlled",
    ...
    D(
        "reference-sample-missing", "Reference sample missing", "Work needing visual or fit comparison cannot be accepted without its reference artifact.", "inspection",
    ...
    D(
        "staging-map-reset", "Staging map reset", "A large movement has invalidated planned WIP locations and access paths.", "staging",
...
FOLLOW_UP_DEFINITIONS = (
    D(
        "narrow-drift-found", "Narrow drift found", "Recalibration shows the measurement concern was limited to one reference.", "inspection",
    ...
    D(
        "route-shortcut-approved", "Route shortcut approved", "Engineering confirms that a copied route step is unnecessary for this work family.", "family",
    ...
    D(
        "spare-fixture-certified", "Spare fixture certified", "The alternate fixture passed its checks and can become a qualified resource.", "fixture",
    ...
    D(
        "family-run-unlocked", "Family run unlocked", "The station is now established for a family of similar work.", "family",
    ...
    D(
        "bulk-lot-released", "Bulk lot released", "The delayed restock arrives as a verified lot with matched paperwork.", "material",
    ...
    D(
        "clean-packet-release", "Clean packet release", "Official labels and packets are ready for controlled batch closeout.", "document",
    ...
    D(
        "finish-window-restored", "Finish window restored", "The serviced process is holding finish quality better than expected.", "workcenter",
    ...
    D(
        "combined-lift", "Combined lift", "The delayed crane window is long enough to move several staged jobs together.", "crane",
    ...
    D(
        "setup-library-update", "Setup library update", "The validated setup sheet can now become controlled reusable shop knowledge.", "document",
    ...
    D(
        "aisles-cleared", "Aisles cleared", "The floor reset has restored clean movement and findable WIP.", "staging",
    ...
    D(
        "clean-room-cleared", "Clean room cleared", "The reset has requalified the controlled area for clean release.", "controlled",
    ...
    D(
        "program-template-saved", "Program template saved", "The reserved programming window produced a reusable controlled template.", "software",
    ...
    D(
        "gauge-method-locked", "Gauge method locked", "The extra measurement work produced an accepted method for the feature family.", "gauge",
    ...
    D(
        "operator-qualified", "Operator qualified", "The newer operator can now cover the same capability independently.", "worker",
    ...
    D(
        "process-tweak-validated", "Process tweak validated", "The method improvement worked and can be extended to similar stations.", "family",
    ...
    D(
        "rack-recovery-sprint", "Rack recovery sprint", "Temporary racks created a clean lane for closing and moving finished WIP.", "staging",
    ...
    D(
        "batch-data-accepted", "Batch data accepted", "The batch record supports accepting the grouped work together.", "batch",
    ...
    D(
        "clamp-marks-found", "Clamp marks found", "Running through weak air pressure left visible marks on affected work.", "shop",
    ...
    D(
        "covered-work-reopened", "Covered work reopened", "Inspectors require covered work to be reopened before acceptance.", "controlled",
    ...
    D(
        "wrong-revision-loaded", "Wrong revision loaded", "A cached or floor-controlled file was not the current revision.", "document",
    ...
    D(
        "phantom-stock-confirmed", "Phantom stock confirmed", "The missing inventory is real and the family cannot support every planned start.", "material",
    ...
    D(
        "fit-check-failed", "Fit check failed", "Downstream fitting found that rough release damaged the planned fit sequence.", "family",
    ...
    D(
        "cure-failure-found", "Cure failure found", "The forced part failed a later cure or bond check.", "family",
    ...
    D(
        "vacuum-trace-failed", "Vacuum trace failed", "The monitoring record crossed the allowed process limit.", "fixture",
    ...
    D(
        "waste-lane-blocked", "Waste lane blocked", "Interim carts now obstruct material movement through the shop.", "waste",
    ...
    D(
        "sticker-audit-hit", "Sticker audit hit", "A calibration-sticker problem was found after work had already moved on.", "tool",
    ...
    D(
        "weather-cleared-early", "Weather cleared early", "The exposed stations can reopen sooner than the revised weather plan expected.", "workcenter",
    ...
    D(
        "setup-mismatch-found", "Setup mismatch found", "The receiving machine's setup datum does not match the moved work's plan.", "workcenter",
    ...
    D(
        "echo-slack-pocket-found", "ECHO slack pocket found", "The reshuffle exposed useful idle capacity hidden by the old queue.", "critical",
    ...
    D(
        "replacement-handoff-check", "Replacement handoff check", "The replacement operator's work needs review before the next dependency trusts it.", "worker",
    ...
    D(
        "returning-worker-shortcut", "Returning worker shortcut", "The returning operator knows a faster safe setup for the paused work.", "worker",
